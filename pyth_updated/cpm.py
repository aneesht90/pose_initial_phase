...
class CPM:
    def __init__(self, config):
        ...
        self.labels = tf.placeholder(
                dtype = tf.float32,
                shape = (self.batch_size, config.fm_height, config.fm_width, self.joint_count_itop))
        self.coords = tf.placeholder(
                dtype = tf.float32,
                shape = (self.batch_size, self.joint_count_itop * 2))
    ...
    def cnn_fc(self, input_, is_train, name):
      ...
      with tf.variable_scope(name) as scope:
        ...
        fc8 = self.final_fc_layer(fc7, \
            self.joint_count_itop * 2, 'fc8', trainable)
    ...
    #         self.joint_count_itop * 2, 'final_fc', trainable)
    #
    #   return final_fc
    ...
    def _fm_summary(self, predicts):
      with tf.name_scope("fcn_summary") as scope:
          self._image_summary(self.labels, self.joint_count_itop)
          tmp_predicts = tf.nn.relu(predicts)
          self._image_summary(tmp_predicts, self.joint_count_itop)
